...
namespace layers
{
DivGradientOper::DivGradientOper(Tensor::SPtr in1, Tensor::SPtr in2,
                                 Tensor::SPtr out, Tensor::SPtr outGrad)
    : Oper({in1, in2, out, outGrad}, createOutputs(in1, in2))
{
}

std::vector<Tensor::SPtr> DivGradientOper::createOutputs(Tensor::SPtr in1,
                                                         Tensor::SPtr in2)
{
...
void DivGradientOper::executeOper(const InputDict& inputs)
{
...
GradientOper::TensorMap DivOper::gradients(Tensor::SPtr output,
                                           Tensor::SPtr outputGrad)
{
    assert(output == mOutputs[0]);
    std::vector<Tensor::SPtr> inputs = getInputs();

    Oper::SPtr gradOper = std::make_shared<DivGradientOper>(
...
Tensor::SPtr div(Tensor::SPtr t1, Tensor::SPtr t2)
{
    ...
    Oper::SPtr oper = std::make_shared<layers::DivOper>(t1, t2);
