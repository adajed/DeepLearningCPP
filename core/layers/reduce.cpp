...
void ReduceBackLayer::execute(const InputDict& inputs)
{
...
void ReduceBackGradientLayer::execute(const InputDict& inputs)
{
    Tensor::SPtr outputGrad = mInputs[2].lock();
    outputGrad->eval(inputs);

    float* in = getInputs()[0]->getMemory().getValues();
    float* out = getInputs()[1]->getMemory().getValues();
    float* outGrad = getInputs()[2]->getMemory().getValues();
...
void ReduceFrontLayer::execute(const InputDict& inputs)
{
...
void ReduceFrontGradientLayer::execute(const InputDict& inputs)
{
    Tensor::SPtr outputGrad = mInputs[2].lock();
    outputGrad->eval(inputs);
