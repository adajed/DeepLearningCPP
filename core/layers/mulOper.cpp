...
{
   public:
    MulGradientOper(Tensor::SPtr in1, Tensor::SPtr in2, Tensor::SPtr out, Tensor::SPtr outGrad)
...
GradientOper::TensorMap MulOper::gradients(Tensor::SPtr output, Tensor::SPtr outputGrad)
{
    assert(output == mOutputs[0]);
    std::vector<Tensor::SPtr> inputs = getInputs();

    Oper::SPtr gradOper =
        std::make_shared<MulGradientOper>(inputs[0], inputs[1], output, outputGrad);
